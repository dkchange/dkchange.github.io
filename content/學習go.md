...
[資料來源：Go 語言教學影片（YouTube）](https://www.youtube.com/watch?v=OSPNUKoN81o&list=PLq9Ra239pNZC0MgMN4j6ZiPHv_c0UPnBX)
...
- Python 簡單但太慢
- Java 太複雜
- C/C++ 複雜、編譯慢（編譯慢對雲端部署來說，是個相當大的缺點）

# 特性

- 強型別靜態型別（99％的變數都會在編譯時被宣告）
- 優良的社群
- 簡單
- 編譯快
- 垃圾回收
...
```console
//設定 GOROOT 目前發現會有版本上的問題，所以能避免就盡量避免
export GOROOT=/usr/local/go
export PATH=$PATH:$GOROOT/bin
//用來放置第三方套件的地方，第一個 GOPATH 設定用來放套件，之後的就依規定裝在 go 工作目錄中
...
- 依照宣告的位置與大寫小寫分三種作用域
  ...
  6. go 對資料型態的轉換非常嚴格，例如不同位元的數字相加，go 不會自動幫你轉換，你必須自己轉
...
- iota
  ...
  - 要注意 iota 從零開始，而其他變數例如 int 的 zero value 是 0，預防方式如下
    ...
    ```go
      // 預設第一個元素代表 error
      const (
        errorList = iota // 或者 _ = iota，compiler 知道 _ 代表不重要的變數
        catList
        dogList
        snakeList
      )
...
- 變數名的長短，反映變數的生命，例如 for 迴圈裡面通常用變數 i
- camelCase 當變數命名有 HTTP 這種縮寫時，請保持 HTTP 都是大寫 useHttp(X) useHTTP(O)
- 不要用底線 `_` 這些來命名
  ...
  - integers
    - signed integers
      - int（預設）不管環境為何，至少會是 32 位元，根據系統不同有可能會是 64 或 128
        ...
        ```
    - unsigned integers
      - uint16 只有正整數（第一個字母 u 代表 unsigned）
        ```go
            var n uint8;
            // 0~255
        ```
...
- text
  - string
    ...
    - 用 **+** 來連結字串
    - 可被轉換成 []byte
    - 取字母取出來的會是 ASCII code
      ```go
      s:= "hello"
      fmt.Println(string(s[1])) //e
      ```
  - rune
    - （預設）UTF32，注意 UTF32 的字母不一定是 32 bits 的長度
    - alias for int32
...
- collection
  ...
  - Maps
    ...
    - 如果元素不存在 map，回傳預設值為 0
      ...
      ```go
        ...
        _,ok := statePopulations["Ohio"] //_是即使這變數沒用到，go也會忽略，ok是習慣命名
  ...
  ```go
    func main(){
      // 通常用在生命週期短的變數，例如你可能要格式化 api 吐的資料結構
...
- Switch case
  - switching with no tag（非固定值的意思）
    ...
    ```
  - 如果要其他語言 falling through 的用法也可以
...
- Panic
- 在 go 沒有 exception，因為在很多語言的 exception 在 go 裡面都算正常，例如要打開一個不存在的檔案，在 go 是 return error value 而不是 exception
...
- 把它想作是變數的一個附加欄位，裡面放的值一定是記憶體位址
- creating pointer

```go
  func main(){
    var a int =42
    var b *int = &a //一個數字的指標，指向 a 位址，用 & 位址運算符拿出位址
...
- pointer arithmetic
  ```go
      ...
      b := &a[0]
      c := &a[1]
      fmt.Printf("%v %p %p\n",a,b,c) // [ 1 2 3 ] 0x1040a124 0x1040a128 可知 b 和 c 元素之間差 4 個 bytes
    }
  ```
  在其他的語言你可能會這樣做
  ```go
      ...
      a := [3]int{1,2,3}
      b := &a[0]
      c := &a[1]-4 //go 不能這樣做記憶體的運算，因為這樣會讓程式變複雜，如果你真的要這麼做，官方有 unsafe package
      fmt.Printf("%v %p %p\n",a,b,c) //error
    }
  ```
- new function
  ```go
    func main(){
      ...
      ms = new(myStruct) //用 new 的話沒辦法像上面一樣初始值，所以他的 zero value 是 0 //&{0}
      (*ms).foo = 42 //()是因為 . 操作符的優先權高，避免取到 ms.foo 的 pointer
  ...
  ```go
    func main(){
      ...
      ms = new(myStruct)
      ms.foo = 42
      fmt.Println(ms.foo) //可以省略星號和括號，因為 go 知道你要取值
    }
  ```
...
- 回傳指標變數
  ...
  func sum() *int{
    result := 0
    // 在很多語言並不允許這樣做，因為 function 執行完的時候，釋放變數，此時你回傳的記憶體位址並不能確定值是正確不變的，但在 go 如果他發現你回傳記憶體位址，他會將此變數提昇到 share memory，也就是我們說的 heap 裡面
    return &result
  }
  ```
- 回傳參數名宣告
  ```go
  //這種方式不常用，因為如果 function 太長，你還要回來看他回傳什麼，可讀性沒比較好
  func sum() (result int){
    result = 12
    return;
  }
...
- struct 裡面會放資料
  ...
  // 這作法很強大，因為你不需要在設計階段就抽出個 interface，因為你隨時都可以加個方法進一個資料型態，只要你可以加方法進去，就代表隨時都可以加個 interface
  // 將 Write依附在 cw，讓 ConsoleWriter 有 write 這個方法
...
- embedding interface（組合 interface）
...
- 類型轉換
  - empty interface 沒有方法的 interface，所以所有的類型都實作這個界面，有點像是 any？

  ```go
    // 然而實際上這界面沒有方法，所以你要不就是利用反射，要不就是要做類型轉換，才能知道你用的是什麼物件，才能運用這個物件
...
- 實例值和指標
  ...
  ```go
    ...
    func main(){
      var wc WriterCloser = &myWriterCloser() // 加上 & 其 method 就可以是 value receiver 或 pointer receiver，但如我們所說，操作 pointer 的時候要小心改到資料
      fmt.Println(wc)
    }
    type Writer interface {
      Write([]byte) (int,error)
...
```go
 func main(){
   ...
   msg = "goodbye"
   time.Sleep(100*time.Millisecond)// 因為 goroutine 會來不及執行，所以加這句
 }
 // goodbye 你可以看出，即便線程是在另一個堆，closure 可以仍取到外面的變數，但是這樣有個缺點，this is what we call race condition，你可以發現 goroutine 在 sleep 之後執行
 // go run -race src/main.go 可以檢查是否有 race condition
```
...
- mutex
  - 同步多個線程
    ...
    func main(){
      ...
      fmt.Printf("Threads:%v\n",runtime.GOMAXPROCS(-1))//印出上一個設定線程的數量，在預設上，go 會預設以電腦 CPU 核心數目相同的線程
    }
    ```
    ...
    // 以上所有的作法都讓 goroutine 的好處消失，變成單線程的程式，這樣倒不如直接不用 goroutine，一般來說多線程也都是在處理不同的事，然而萬一真的有共享的資料，mutex 就是用在這時候
    ```
...
```go
  var wg = sync.WaitGroup{}
  func main(){
    ch:=make(chan int) // 初始化 channel 所能接收或發出的資料型態
    wg.Add(2)
    go func(ch <-chan int){ // receiving data
      i:=<- ch
      fmt.Println(i)
      wg.Done()
    }(ch)
    go func(ch chan<- int){ // sending data
      ch<-42
      wg.Done()
    }(ch)
    wg.Wait()
...
- close channel
  ...
  ```go
    var wg = sync.WaitGroup{}
    func main(){
      ch:=make(chan int,50) // 初始化 channel 所能接收或發出的資料型態
      wg.Add(2)
      go func(ch <-chan int){ // receiving data
        for {
          if i,ok :=<-ch;ok{
...
- select
  ...
  ```go
    ...
    func logger(){
      for {
        select{ // 整個段落會阻塞，直到我們收到需要的 channel
          case entry := <-logCh:
            fmt.Printf("%v - [%v]%v\n",entry.time.Format("2006-01-02",entry.severity,entry.message))
          case <-doneCh:
            break;
